            ...
            with dpg.table(width=1440, height=640, header_row=False):
                ...
                with dpg.table_row():
                    with dpg.table_cell():
                        ...
                        with open('data//selector.txt', encoding="utf-8") as f:
                            lines = f.readlines()
                            for line in lines:
                                dpg.add_text(line, indent=20)

                        dpg.add_spacer(height=20)
                        with open('data//selectorExamples.txt', encoding="utf-8") as f:
                            lines = f.readlines()
                            i = 0
                            for line in lines:
                                if i == 0 or i == 2:
                        ...
                        with dpg.drawlist(width=1440, height=500, tag="selection_animation"):
                            ...
                            with dpg.draw_layer():
                                dpg.draw_line((48 + self.x_move, 50 + 2 * self.y_offset),
                                              (653 + self.x_move, 50 + 2 * self.y_offset), color=self.chromo_color,
                                              thickness=5)
                                dpg.draw_line((50 + self.x_move, 50 + 2 * self.y_offset),
                                              (50 + self.x_move, 103 + 2 * self.y_offset), color=self.chromo_color,
                                              thickness=5)
                                dpg.draw_line((50 + self.x_move, 100 + 2 * self.y_offset),
                                              (653 + self.x_move, 100 + 2 * self.y_offset), color=self.chromo_color,
                                              thickness=5)
                                x = 100 + self.x_move
                                y = 50
                                allel_x = 63 + self.x_move
